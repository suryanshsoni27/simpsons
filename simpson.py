...
def _integrate(t, n, _f):
    ...
    if(t == None or n == None or type(_f) == int or type(_f)== float or type(_f) == int):
        return None
    elif(type(t)!= float or type(n)!= int):
        return None
    elif(t <= 0 or n <3):
        return None
    else:
        ...
        while(abs((simpsonsNew -  simpsonsOld)/simpsonsNew)> epsilon):
             simpsonsOld = simpsonsNew
             w = (highbound - lowbound)/s
             wn = 0
             sum = 0 
             count = 0 
             while(wn <= highbound):
                 if(count == lowbound or count == highbound):
                     sum = sum +_f(lowbound+wn,n)      
                 elif(count % 2 != 0):
                     sum = sum + 4*_f(lowbound+wn,n)
                 elif(count%2 == 0): 
                     sum = sum + 2*_f(lowbound+wn,n)   
                 else:   
                     sum = sum + 0 
                 wn += w 
                 count = count + 1 
